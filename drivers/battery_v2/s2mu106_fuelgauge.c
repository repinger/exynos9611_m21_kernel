...
static int s2mu106_fix_rawsoc_reset_fg(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: UI SOC = %d\n", __func__, value.intval);
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: Finish\n", __func__);
...
static int s2mu106_get_batcap_ocv(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: data0 (%d) data1 (%d) \n", __func__, data[0], data[1]);
...
static int s2mu106_get_rawsoc(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	if (((por_state != 0x00) || (reg_1E != 0x03)) ||
		...
		dev_dbg_once(&fuelgauge->i2c->dev, "%s: FG reset\n", __func__);
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: current_soc (%d), compen_soc (%d), "
...
#else
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: current_soc (%d), previous_soc (%d), FG_mode(%s)\n",
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: UI SOC = %d, is_charging = %d, avg_vbat = %d, "
	...
	if (is_swelling_status) {
		if (fuelgauge->mode == HIGH_SOC_VOLTAGE_MODE) {
			...
			dev_dbg_once(&fuelgauge->i2c->dev, "%s: FG is in current mode\n", __func__);
		}
	} else {
		if (((fuelgauge->is_charging == true) && (fuelgauge->ui_soc >= 98) && (avg_current > 50)) ||
			...
			if (fuelgauge->mode == CURRENT_MODE) { /* switch to VOLTAGE_MODE */
				...
				dev_dbg_once(&fuelgauge->i2c->dev, "%s: FG is in high soc voltage mode\n", __func__);
			}
...
batcap_learn_init:
	...
	if (fuelgauge->temperature > fuelgauge->low_temp_limit) {
		if ((avg_vbat < fuelgauge->low_voltage_limit) &&
			...
			if (low_voltage_limit_cnt >= fuelgauge->low_voltage_limit_cnt) {
				dev_dbg_once(&fuelgauge->i2c->dev, "%s: Low voltage WA in normal temperature.\n", __func__);
		...
		if ((avg_vbat < fuelgauge->low_voltage_limit_lowtemp) &&
			...
			dev_dbg_once(&fuelgauge->i2c->dev, "%s: Low voltage WA in Low temperature. Make UI SOC 0\n", __func__);
	...
	if (touch_low_voltage &&
		...
		dev_dbg_once(&fuelgauge->i2c->dev,
...
static int s2mu106_get_current(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	if (s2mu106_read_reg(fuelgauge->i2c, S2MU106_REG_RCUR_CC, data) < 0)
		return -EINVAL;
	compliment = (data[1] << 8) | (data[0]);
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: rCUR_CC(0x%4x)\n", __func__, compliment);
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: current (%d)mA\n", __func__, curr);
...
ocv_soc_mapping:
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: soc (%d), ocv (%d)\n", __func__, soc, ocv);
	return ocv;
}
...
static int s2mu106_get_avgcurrent(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	if (s2mu106_read_reg(fuelgauge->i2c, S2MU106_REG_MONOUT, data) < 0)
		goto err;
	compliment = (data[1] << 8) | (data[0]);
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: MONOUT(0x%4x)\n", __func__, compliment);
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: avg current (%d)mA\n", __func__, curr);
	...
	struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	if ((cnt < 10) && (curr < 0) && (fuelgauge->is_charging) &&
		...
		curr = 1;
		cnt++;
		dev_dbg_once(&fuelgauge->i2c->dev, "%s: vcell (%d)mV, modified avg current (%d)mA\n",
				 __func__, vcell, curr);
	}
...
static int s2mu106_get_vbat(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: vbat (%d)\n", __func__, vbat);
...
static int s2mu106_get_avgvbat(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: avgvbat (%d)\n", __func__, avg_vbat);
...
bool s2mu106_fuelgauge_fuelalert_init(struct i2c_client *client, int soc)
{
	...
	dev_dbg_once(&client->dev, "%s: irq_reg(%02x%02x) irq(%d)\n",
		...
		struct s2mu106_fuelgauge_data *fuelgauge, int capacity_max)
{
	...
	if (new_capacity_max < (fuelgauge->pdata->capacity_max -
		...
		dev_dbg_once(&fuelgauge->i2c->dev, "%s: set capacity max(%d --> %d)\n",
...
static void s2mu106_fg_isr_work(struct work_struct *work)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s : fg_alert_status(0x%x)\n",
...
static irqreturn_t s2mu106_fg_irq_thread(int irq, void *irq_data)
{
	...
	dev_dbg_once(&fuelgauge->i2c->dev, "%s: fg_irq(0x%x)\n",
		__func__, fg_irq);
