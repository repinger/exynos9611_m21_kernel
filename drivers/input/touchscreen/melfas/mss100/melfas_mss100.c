...
static void mms_run_rawdata(struct mms_ts_info *info, bool on_probe)
{
	...
	if (mms_run_test(info, MIP_TEST_TYPE_CM)) {
		input_silence(true, &info->client->dev, "%s cm error\n", __func__);
		goto out;
	}
	minority_report_sync_latest_value(info);

	if (mms_run_test(info, MIP_TEST_TYPE_CP)) {
		input_silence(true, &info->client->dev, "%s cp error\n", __func__);
		goto out;
	}

	if (mms_run_test(info, MIP_TEST_TYPE_CP_SHORT)) {
		input_silence(true, &info->client->dev, "%s cp short error\n", __func__);
		goto out;
	}

	if (mms_run_test(info, MIP_TEST_TYPE_CP_LPM)) {
		input_silence(true, &info->client->dev, "%s cp lpm error\n", __func__);
		goto out;
	}

	if (on_probe) {
		if (mms_run_test(info, MIP_TEST_TYPE_CM_ABS)) {
			input_silence(true, &info->client->dev, "%s cm abs error\n", __func__);
			goto out;
		}

		if (mms_run_test(info, MIP_TEST_TYPE_CM_JITTER)) {
			input_silence(true, &info->client->dev, "%s cm_jitter error\n", __func__);
			goto out;
		}
	}
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (mms_sysfs_create(info)) {
		input_silence(true, &client->dev, "%s [ERROR] mms_sysfs_create\n", __func__);
		ret = -EAGAIN;
